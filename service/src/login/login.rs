...
/// 登录业务
pub async fn login(account: String, password: String, db: &DatabaseConnection) -> Result<UserInfo, ResultErr> {
    ...
    if let Some(user) = user {
        ...
        let verify = Argon2::default()
            .verify_password(password.as_bytes(), &PasswordHash::new(&hash_password).unwrap());

        if verify.is_ok() {
            // 生成token 
            // todo!("生成secret");
            let access_exp = Local::now() + Duration::hours(2);
            let access_token = get_token(user.id, access_exp.timestamp(), account.clone(), "aaaa".into())
                .map_err(|_e| { ResultErr::BizErr { msg: "生成access_token失败".into() }})?;
            let refresh_exp = Local::now() + Duration::days(7);
            let refresh_token = get_token(user.id, refresh_exp.timestamp(), account.clone(), "refresh_secret".into())
                .map_err(|_e| { ResultErr::BizErr { msg: "生成refresh_token失败".into() }})?;
            let info = UserInfo {id: user.id, account, name: user.name, access_token, refresh_token };
            return Ok(info);
        }
    }
    
    
    Err(ResultErr::BizErr { msg: "登录失败".into() })
}
...
mod tests {
    ...
    fn test_argon2() {
        ...
        // 验证
        let verify = Argon2::default().verify_password(password, &PasswordHash::new(&hash_password).unwrap());
        assert!(verify.is_ok());

        let n_p = b"hU980512";
        let h_e_pwd = gen_hash_pwd(n_p);
        let verify = Argon2::default().verify_password(password, &PasswordHash::new(&h_e_pwd).unwrap());
    ...
    fn gen_hash_pwd(password: &[u8]) -> String {
        ...
        println!("hash_password is {}, len is {}", hash_password, hash_password.len());
        hash_password
    }
}